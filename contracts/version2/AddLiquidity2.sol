...
constructor(address _capl, address _usdc, address _pair) public {
  ...
  (caplUsdcPair) = (_pair);
      ...
      function addLiquidityCapl(uint256 tokenAmount, uint256 duration, address _address) external returns( uint256 _amountA , uint256 _amountB ,uint256 _amounts){
        ...
        (uint256 amountA , uint256 amountB ,uint256 amounts) = addLiq(capl,usdc, tokenAmount.div(2), amount, msg.sender, duration);

        IERC20(capl).balanceOf(address(this)) > 0 && IERC20(capl).transfer(_address, IERC20(capl).balanceOf(address(this)));
        IERC20(usdc).balanceOf(address(this)) > 0 && IERC20(usdc).transfer(_address, IERC20(usdc).balanceOf(address(this)));

        return(amountA, amountB, amounts);
      }
      
       function addLiquidityUsdc(uint256 tokenAmount, uint256 duration, address _address) external returns(uint256 _amountA , uint256 _amountB ,uint256 _amounts){
        IERC20(usdc).transferFrom(msg.sender, address(this),tokenAmount) ; 
         ...
         (uint256 amountA , uint256 amountB ,uint256 amounts) = addLiq(usdc,capl, tokenAmount.div(2), amount, msg.sender, duration);
      ...
      function addLiquidityBoth(uint256 tokenAmountcapl, uint256 duration) external returns(uint256 _amountA , uint256 _amountB ,uint256 _amounts){
        ...
        IERC20(capl).balanceOf(address(this)) > 0 && IERC20(capl).transfer(msg.sender, IERC20(capl).balanceOf(address(this)));
        IERC20(usdc).balanceOf(address(this)) > 0 && IERC20(usdc).transfer(msg.sender, IERC20(usdc).balanceOf(address(this)));
